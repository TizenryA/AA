...
### 🚨 2026-02-24 配置崩溃重大事故
- **现象**: 修改 openclaw.json 后重启 Gateway 导致配置文件损坏，系统命令（包括 uninstall）全部失效。
- **原因**: 在 Gateway 运行期间直接用脚本暴力修改核心 JSON 文件可能导致写入竞争或格式损坏。
- **教训**: **严禁在未备份的情况下直接操作 openclaw.json！！** 
- **修复方法**: 必须手动恢复 .bak 备份文件。
